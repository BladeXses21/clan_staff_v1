...
from src.models.shop import shop_item
...
class ClanService:
    ...
    async def drop_menu(self, interaction: Interaction, ctx: ApplicationContext = None):
        ...
        try:
            ...
            if ctx.message is None:
                await ctx.response.send_message(embed=BaseEmbed('***```Выберите пользователя```***'), view=view, delete_after=160)
            else:
                await ctx.response.edit_message(embed=BaseEmbed('***```Выберите пользователя```***'), view=view, delete_after=160)
        ...
        async def menu_callback(interact: Interaction):
            ...
            async def edit_time_callback(inter: Interaction):
                ...
                await inter.response.send_message(embed=BaseEmbed(
                ...
                if msg.author.id == inter.user.id:
                    try:
                        ...
                        await inter.followup.send(embed=BaseEmbed(f'Успешно изменено на: {msg.content} с. пользователю <@{member_id}>'), ephemeral=True)

                        await msg.delete()

                        logger.info(f'{msg.author} изменил время ивентов {member_id} на {msg.content}')

                    except ValueError:
                        logger.warn(f'{msg.author}, ввів погане значення для часу.')
                        await inter.followup.send(embed=BaseEmbed('Не коректное число.'), ephemeral=True, delete_after=10)
                        await msg.delete()
                else:
                    pass
            ...
            async def edit_event_callback(inter: Interaction):
                ...
                if msg.author.id == inter.user.id:
                    try:
                        ...
                        await inter.followup.send(embed=BaseEmbed(f'Успешно изменено на: {msg.content} ивентов. пользователю <@{member_id}>'), ephemeral=True)
                        await msg.delete()

                        logger.info(f'{msg.author} изменил ивенты {member_id} на {msg.content}')

                    except ValueError:
                        await inter.followup.send(embed=BaseEmbed('Не коректное число.'), ephemeral=True)
                        await msg.delete()
                else:
                    pass
            ...
            async def edit_butterfly_callback(inter: Interaction):
                ...
                if msg.author.id == inter.user.id:
                    try:
                        ...
                        await inter.followup.send(embed=BaseEmbed(f'Успешно изменено на: {msg.content} бабочек. пользователю <@{member_id}>'), ephemeral=True)
                        await msg.delete()

                        logger.info(f'{msg.author} изменил бабочки {member_id} на {msg.content}')
            ...
            async def edit_xp_callback(inter: Interaction):
                ...
                if msg.author.id == inter.user.id:
                    try:
                        ...
                        await inter.followup.send(embed=BaseEmbed(f'Успешно изменено на: {msg.content} опыта. пользователю <@{member_id}>'), ephemeral=True)
                        await msg.delete()

                        logger.info(f'{msg.author} изменил опыт {member_id} на {msg.content}')
            ...
            async def shop_menu(inter: Interaction):
                ...
                async def select_option(inte: Interaction):
                    ...
                    if user_butterfly < shop_item.SHOP_ITEM_AMOUNT[int(select_menu.values[0])]:
                        return await inter.followup.send(embed=BaseEmbed("***```У вас не достаточное количество валюты для покупки.```***"), delete_after=10, ephemeral=True)
                    ...
                    if select_menu.values[0] in ["4", "8", "11", "12", "13", "14", "16", "20"]:
                        ...
                        await inter.followup.send(embed=BaseEmbed(values_content), delete_after=120)
                        return await inte.client.get_channel(SHOP_CHANNEL_ID).send(embed=BaseEmbed(f'{inte.user.mention}\n{values_content}'))

                    if select_menu.values[0] == '15':
                        item_num = shop_item.get_random_item()
                        get_random_item = shop_item.RAND_ITEM_RESPONSE[item_num]
                        await inter.followup.send(embed=BaseEmbed(get_random_item), delete_after=120)
                        return await inte.client.get_channel(SHOP_CHANNEL_ID).send(embed=BaseEmbed(f'{inte.user.mention} выпал предмет под номером {str(item_num)}\n{get_random_item}'))
                        ...
                        if msg.author.id == inte.user.id:
                            await inte.client.get_channel(SHOP_CHANNEL_ID).send(embed=BaseEmbed(f'{inte.user.mention}\n{values_content}\n{msg.content}'))
