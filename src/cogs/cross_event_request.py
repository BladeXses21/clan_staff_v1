...
from config import (
    TENDERLY_ID, META_ID, DARKNESS_ID,
    HATORY_ID, CLAN_STAFF, STOP_WORD,
    SERVERS, SWEETNESS_ID, OWNER_IDS
)
...
from extensions.funcs import (
    sum_event_time, is_member_in_voice, get_guilds_list_async,
    get_staff_list_async, get_staff_event_list, remove_clan_staff_response,
    add_clan_staff_response, get_event_history, get_fault,
    get_quest_list, quest_info, xp_to_lvl
)

from src.services.clan import ClanService
...
class CrossEventsMode(BaseCog):
    ...
    async def rem_quest(self, ctx, member: discord.Member, name: str):
        ...
        await ctx.send(embed=BaseEmbed(f'***```Квест {name} {xp[name]} xp. {timer[name]} timer. у {member.name} был удален ```***'))
    ...
    async def add(self, ctx, member: discord.Member):
        ...
        if member is None:
            return await ctx.send(
                embed=BaseEmbed(f'***```{author_name}, вы не указали пользователя```***'),
                delete_after=60
            )

        if cross_server_system.find_guild_id(guild_id=guild) is False:
            return await ctx.send(
                embed=BaseEmbed(f'***```{author_name}, клан стафф не существует на этом сервере```***'),
                delete_after=60
            )

        if cross_event_system.is_clan_staff(guild_id=guild, clan_staff_id=member.id) is True:
            return await ctx.send(
                embed=BaseEmbed(f'***```{author_name}, пользователь {member.name}, уже есть в списке clan staff```***'),
                delete_after=60
            )
        # <end>
        await event_channel.set_permissions(member, overwrite=overwrite)
        logger.info(f'{ctx.author}, use !staff add to {member}')
        ...
        await ctx.send(embed=BaseEmbed(add_clan_staff_response(member)))
    ...
    async def kick(self, ctx, member: discord.Member):
        ...
        if cross_event_system.is_clan_staff(guild_id=guild, clan_staff_id=member.id) is False:
            return await ctx.send(
                embed=BaseEmbed(f'***```{author_name}, пользователя {member.name}, нет в списке clan staff```***'),
                delete_after=60
            )

        await event_channel.set_permissions(member, overwrite=None)
        logger.info(f'{ctx.author}, use !staff kick to {member}')
        ...
        await ctx.send(embed=BaseEmbed(remove_clan_staff_response(member)))
        return await ctx.message.delete()
    ...
    async def clear(self, ctx):
        ...
        msg = await ctx.send(
            embed=BaseEmbed(f'{ctx.author.name}, подтвердите очистку статистики.'),
            view=def_view,
            delete_after=60
        )
        ...
        async def accept_callback(interaction: discord.Interaction):
            ...
            return await msg.edit(
                embed=BaseEmbed(f'***```{author_name}, вы успешно очистили статистику clan staff на {ctx.guild.name}```***'),
                delete_after=60,
    ...
    async def request(self, interaction: discord.Interaction,
        ...
        logger.info(f'{interaction.user} вызвал комаду /event request {event_num} {users_count} {comment}')
        ...
        if interaction.user.id not in member_ids:
            return await interaction.response.send_message(
                embed=BaseEmbed(f'***```Вы должны быть в голосовом канале клана.```***'),
                ephemeral=True
            )
        ...
        async def accept_callback(ctx):
            ...
            if cross_event_system.is_clan_staff(server.id, user.id) is False:
                return await ctx.response.send_message(
                    embed=BaseEmbed(f'***```{user.name}, тебя нет в clan staff```***'),
                    ephemeral=True
                )

            if cross_event_system.is_event_completed(server.id, user.id) is False:
                return await ctx.response.send_message(
                    embed=BaseEmbed(f'***```{user.name}, ты не закончил прошлый ивент.```***'),
                    ephemeral=True
                )
        ...
        async def pass_callback(ctx):
            ...
            if cross_event_system.is_clan_staff(server.id, user.id) is False:
                return await ctx.response.send_message(embed=BaseEmbed(f'***```{user.name}, тебя нет в clan staff```***'), ephemeral=True)

            if request_member_id != message.id:
                return await ctx.response.send_message(embed=BaseEmbed(f'***```{user.name}, это не ваш ивент.```***'), ephemeral=True)

            await ctx.response.send_message(embed=BaseEmbed(
    ...
    async def profile(self, interaction: discord.Interaction):
        logger.info(f'{interaction.user.name} use command /staff profile')
    ...
    async def his(self, ctx: ApplicationContext, member: discord.Member = None):
        try:
            if member is not None:
                logger.info(f'{ctx.author.name} use command !staff his on {member.name}')
            ...
            await ctx.send(embed=BaseEmbed('***```История участника пуста или его нет в списке.```***'), delete_after=30)
            return await ctx.message.delete()
        try:
            if member is None:
                logger.info(f'{ctx.author.name} use command !staff his')
    ...
    async def clear_his(self, ctx: ApplicationContext):
        ...
        return await ctx.send(embed=BaseEmbed(f'***```История участников на сервере {ctx.guild.name}, была сброшена.```***'), delete_after=30)
    ...
    async def warn(self, ctx: ApplicationContext, member: discord.Member, reason: str, f_type: str):
        ...
        return await ctx.send(embed=BaseEmbed(f'***```{member.name}, получил выговор по причине {reason} с типом {f_type}```***'))
    ...
    async def fault(self, ctx: ApplicationContext, member: discord.Member = None):
        try:
            if member is None:
                logger.info(f'{ctx.author.name} use command !staff fault')
            ...
            await ctx.send(embed=BaseEmbed('***```История участника пуста или его нет в списке.```***'), delete_after=30)
        try:
            logger.info(f'{ctx.author.name} use command !staff fault on {member.name}')
    ...
    async def dr(self, ctx: ApplicationContext, member: discord.Member, *args: str):
        birthday = ' '.join(args)
        if member is None:
            return await ctx.send(embed=BaseEmbed('***```Ошибка | Форма команды: !staff dr [id/@link] [new_birthday]```***'), delete_after=10)
        # if len(args) <= 2:
        ...
        await ctx.send(embed=BaseEmbed(f'***```{member.name}, успешно обновлен на {birthday}```***'), delete_after=30)
        return await ctx.message.delete()
    ...
    async def av(self, ctx: ApplicationContext, member: discord.Member, avatar_url: str):
        if member is None:
            return await ctx.send(embed=BaseEmbed('***```Ошибка | участник не указан```***'), delete_after=10)
        if avatar_url is None:
            return await ctx.send(embed=BaseEmbed('***```Ошибка | avatar_url не указан```***'), delete_after=10)
        cross_event_system.update_avatar(guild_id=ctx.guild.id, clan_staff_id=member.id, new_avatar=avatar_url)
        await ctx.send(embed=BaseEmbed(f'***```{member.name}, аватар обновлен```***'), delete_after=10)
        return await ctx.message.delete()
    ...
    async def back(self, ctx: ApplicationContext, member: discord.Member, background_url: str):
        ...
        if background_url is None:
            return await ctx.send(embed=BaseEmbed('***```Ошибка | background_url не указан```***'), delete_after=10)
        cross_event_system.update_background(guild_id=ctx.guild.id, clan_staff_id=member.id, new_background=background_url)
        await ctx.send(embed=BaseEmbed(f'***```{member.name}, background обновлен```***'), delete_after=10)
        return await ctx.message.delete()
    ...
    async def unwarn(self, ctx: ApplicationContext, member: discord.Member, warn_index: int):
        ...
        if warn_index is None:
            return await ctx.send(embed=BaseEmbed('***```Ошибка | номер выговора не указан```***'), delete_after=10)
        fault_system.remove_the_fault(guild_id=ctx.guild.id, clan_staff_id=member.id, warn_index=warn_index)
        return await ctx.send(embed=BaseEmbed(f'***```Выговор: {warn_index} был снят.```***'), delete_after=10)
    ...
    async def new_quest(self, ctx, name: str, timer: int, xp: int):
        ...
        await ctx.send(embed=BaseEmbed(f'***```{ctx.author.name}, квест {name} | {timer} m. | {xp} xp, заряжен```***'))
