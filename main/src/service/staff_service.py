...
class ClanService:
    ...
    async def drop_menu(self, interaction: Interaction, ctx: ApplicationContext = None):
        ...
        try:
            author_id, t_events, t_times, my_add_time, my_curator, my_xp, my_avatar, my_background, my_birthday, my_color = cross_event_system.get_clan_staff(
            ...
            if ctx.message is None:
                await ctx.response.send_message(
                    embed=StaffProfile(member=ctx.author, total_event=t_events, total_time=t_times,
                                       butterfly=my_butterfly, add_time=my_add_time, curator=f"<@{my_curator}>",
                                       xp=my_xp, avatar_img=my_avatar, background_img=my_background,
                                       birthday=my_birthday, color=my_color, lvl=my_lvl, guild_id=guild_id).embed,
                    view=view, delete_after=160)
            else:
            ...
            if ctx.message is None:
                await ctx.response.send_message(embed=DefaultEmbed('***```Выберите пользователя```***'), view=view,
                                                delete_after=160)
            else:
        ...
        async def menu_callback(interact: Interaction):
            if interact.user.id != ctx.author.id:
                return False
            common_view = clan_staff_view.standard_profile_view(drop_down_menu)

            admin_view = clan_staff_view.admin_profile_view(drop_down_menu)

            guild_id = interact.guild.id
            member_id, total_event, total_time, add_time, curator, xp, avatar, background, birthday, color = cross_event_system.get_clan_staff(
                guild_id, drop_down_menu.values[0])
            ...
            if interact.user.id not in OWNER_IDS:
                ...
                await interact.response.edit_message(
                    embed=StaffProfile(member=get_member, total_event=total_event, total_time=total_time,
                                       ...
                                       avatar_img=avatar, background_img=background, birthday=birthday, color=color,
                                       lvl=lvl, guild_id=guild_id).embed,
                    view=common_view)
            else:
                await interact.response.edit_message(
                    embed=StaffProfile(member=get_member, total_event=total_event, total_time=total_time,
                                       butterfly=get_butterfly, add_time=add_time, curator=f'<@{curator}>', xp=xp,
                                       avatar_img=avatar, background_img=background, birthday=birthday, color=color,
                                       lvl=lvl, guild_id=guild_id).embed, view=admin_view)
            ...
            async def edit_time_callback(inter: Interaction):
                ...
                if msg.author.id == inter.user.id:
                    try:
                        ...
                        cross_event_system.update_wasting_time(guild_id=inter.guild.id, clan_staff_id=member_id,
                                                               waisting_time=number)
                        await inter.followup.send(
                            embed=DefaultEmbed(f'Успешно изменено на: {msg.content} с. пользователю <@{member_id}>'),
                            ephemeral=True)
            ...
            async def edit_event_callback(inter: Interaction):
                ...
                if msg.author.id == inter.user.id:
                    try:
                        number = int(msg.content)
                        cross_event_system.update_number_event(guild_id=inter.guild.id, clan_staff_id=member_id,
                                                               sum_event_ends=number)
                        await inter.followup.send(embed=DefaultEmbed(
                            f'Успешно изменено на: {msg.content} ивентов. пользователю <@{member_id}>'), ephemeral=True)
                        await msg.delete()
            ...
            async def edit_butterfly_callback(inter: Interaction):
                ...
                if msg.author.id == inter.user.id:
                    try:
                        number = int(msg.content)
                        save_stats_system.update_butterfly(guild_id=inter.guild.id, clan_staff_id=member_id,
                                                           butterfly=number)
                        await inter.followup.send(embed=DefaultEmbed(
                            f'Успешно изменено на: {msg.content} бабочек. пользователю <@{member_id}>'), ephemeral=True)
                        await msg.delete()
            ...
            async def edit_xp_callback(inter: Interaction):
                ...
                if msg.author.id == inter.user.id:
                    try:
                        number = int(msg.content)
                        cross_event_system.update_xp_counter(guild_id=inter.guild.id, clan_staff_id=member_id,
                                                             xp=number)
                        await inter.followup.send(embed=DefaultEmbed(
                            f'Успешно изменено на: {msg.content} опыта. пользователю <@{member_id}>'), ephemeral=True)
                        await msg.delete()
            ...
            async def quest_menu(inter: Interaction):
                ...
                clr, av = cross_event_system.get_color_and_avatar(guild_id=inter.guild.id,
                                                                  clan_staff_id=int(drop_down_menu.values[0]))
                await inter.response.edit_message(
                    embed=QuestEmbed(member=inter.guild.get_member(int(drop_down_menu.values[0])),
                                     description=get_description, msg_author=inter.user, color=clr, avatar=av).embed,
                    delete_after=90)
            ...
            async def history_menu(inter: Interaction):
                ...
                await inter.response.edit_message(
                    embed=HistoryEmbed(member=inter.guild.get_member(int(drop_down_menu.values[0])),
                                       description=get_description, msg_author=inter.user, color=clr, avatar=av).embed,
                    delete_after=90)
            ...
            async def fault_menu(inter: Interaction):
                ...
                await inter.response.edit_message(
                    embed=FaultEmbed(
                        member=inter.guild.get_member(int(drop_down_menu.values[0])), command_use=inter.user, date=date,
                        reason=reason, fault_type=f_type,
            ...
            async def shop_menu(inter: Interaction):
                ...
                await inter.response.edit_message(embed=StaffShop(inter.user, color=color).embed, view=view_1,
                                                  delete_after=90)
                ...
                async def select_option(inte: Interaction):
                    ...
                    if user_butterfly < shop_item.SHOP_ITEM_AMOUNT[int(select_menu.values[0])]:
                        return await inter.followup.send(
                            embed=DefaultEmbed("***```У вас не достаточное количество валюты для покупки.```***"),
                            delete_after=10, ephemeral=True)

                    save_stats_system.update_butterfly(inte.guild.id, inte.user.id,
                                                       -shop_item.SHOP_ITEM_AMOUNT[int(select_menu.values[0])])
                    ...
                    if select_menu.values[0] in ["4", "8", "11", "12", "13", "14", "16", "20"]:
                        ...
                        return await inte.client.get_channel(SHOP_CHANNEL_ID).send(
                            embed=DefaultEmbed(f'{inte.user.mention}\n{values_content}'))
                    ...
                    if select_menu.values[0] == '15':
                        ...
                        return await inte.client.get_channel(SHOP_CHANNEL_ID).send(embed=DefaultEmbed(
                            f'{inte.user.mention} выпал предмет под номером {str(item_num)}\n{get_random_item}'))
                        ...
                        if msg.author.id == inte.user.id:
                            await inte.client.get_channel(SHOP_CHANNEL_ID).send(content=f'<@&{BLADEXSES_ID}>',
                                                                                embed=DefaultEmbed(
                                                                                    f'{inte.user.mention}\n{values_content}\n{msg.content}'))
