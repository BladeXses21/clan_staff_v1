...
from models.mongo_type import CrossStaffModel, RequestEventModel
...
class CrossEventsSystem(DatabaseSystem):
    ...
    def get_request_msg_id(self, guild_id: int, clan_staff_id: int) -> CrossStaffModel.member_work_this_request:
        res = self.cross_event_mode_collection.find_one({'guild_id': guild_id, 'clan_staff_id': clan_staff_id})
    ...
    def enumeration_events_mode(self, guild_id: int) -> dict[Literal['clan_staff_id'] | Literal['sum_event_ends'], int]:
        dbm = CrossStaffModel(guild_id=guild_id)
    ...
    def get_clan_staff(self, guild_id: int, clan_staff_id: int) -> tuple[
        int, int, int, int, int, int, str, str, str, int]:
        dbm = CrossStaffModel(guild_id=guild_id, clan_staff_id=clan_staff_id)
        res = self.cross_event_mode_collection.find_one(dbm.to_mongo())
        return res['clan_staff_id'], res['sum_event_ends'], res['wasting_time'], res['add_time'], res['curator'], res[
            'xp_counter'], res['avatar'], res['background'], res['birthday'], res[
                   'color']

    def get_lvl_count(self, guild_id: int, clan_staff_id: int) -> CrossStaffModel.lvl:
        dbm = CrossStaffModel(guild_id=guild_id, clan_staff_id=clan_staff_id)
        res = self.cross_event_mode_collection.find_one(dbm.to_mongo())
        return res['lvl']

    def get_xp_count(self, guild_id: int, clan_staff_id: int) -> CrossStaffModel.xp_counter:
        dbm = CrossStaffModel(guild_id=guild_id, clan_staff_id=clan_staff_id)
        res = self.cross_event_mode_collection.find_one(dbm.to_mongo())
        return res['xp_counter']

    def get_color_and_avatar(self, guild_id: int, clan_staff_id: int) -> tuple[
        CrossStaffModel.color, CrossStaffModel.avatar]:
        dbm = CrossStaffModel(guild_id=guild_id, clan_staff_id=clan_staff_id)
        res = self.cross_event_mode_collection.find_one(dbm.to_mongo())
        return res['color'], res['avatar']

    # достает всех ивентеров с базы для таблицы
    def get_event_organizers(self, guild_id: int) -> dict[
        Literal['clan_staff_id'] | Literal['sum_event_ends'], Literal['wasting_time'] | Literal['add_time']]:
        dbm = CrossStaffModel(guild_id=guild_id)
        return self.cross_event_mode_collection.find(
            dbm.to_mongo(),
            {
    ...
    def get_clan_event(self, guild_id: int, message_id: int) -> tuple[
        RequestEventModel.event_num, RequestEventModel.comment, RequestEventModel.clan_name, RequestEventModel.member_send_request]:
