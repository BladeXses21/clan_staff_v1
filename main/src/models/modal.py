...
from config import REQUEST_STAFF_CHANNEL, CURATOR_ROLE_BY_SERVER_ID, png_strip_for_embed, BLADEXSES_ID, \
    CLAN_CREATE_REQUEST_CHANNEL
...
class StaffModal(Modal):
    def __init__(self, ctx: ApplicationContext) -> None:
        ...
        self.ctx = ctx
        self.add_item(
            InputText(label='ID/НИК/ИМЯ И ВОЗРАСТ', placeholder='9541302791235454545, BladeXses#6699, Артур, 20'))
        self.add_item(
            InputText(label='ПОЧЕМУ МЫ ДОЛЖНЫ ВЗЯТЬ ТЕБЯ?', placeholder='...', style=discord.InputTextStyle.long))
        self.add_item(InputText(label='НА КАКУЮ ИЗ ДВУХ ВЕТОК ТЫ ХОЧЕШЬ ВСТАТЬ?',
                                placeholder='Например: Clan Manager или Clan EventMod'))
        self.add_item(InputText(label='УКАЖИ СВОЙ ЧАСОВОЙ ПОЯС.', placeholder='Например: (UTS +3) или +1 от МСК'))
        self.add_item(
            InputText(label='КОЛИЧЕСТВО НАРУШЕНИЙ и ОНЛАЙН НА СЕРВЕРЕ', placeholder='0 нарушений | 00 ч. 00 м.'))
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(content=f'{interaction.user.mention}, заявка успешно отправлена',
                                                embed=emb, ephemeral=True)
        await interaction.client.get_channel(REQUEST_STAFF_CHANNEL[interaction.guild.id]).send(embed=emb)
...
class ClanModal(Modal):
    def __init__(self, ctx: ApplicationContext, *args, **kwargs) -> None:
        ...
        self.ctx = ctx
        self.add_item(InputText(label='ВВЕДИ БУДУЩИХ УЧАСТНИКОВ СВОЕГО КЛАНА мин.10',
                                placeholder='Если ты заполняешь это на Tenderly, то этих людей ты должен пригласить на сервер.',
                                style=discord.InputTextStyle.long))
        self.add_item(InputText(label='УКАЖИ ЛИДЕРА КЛАНА ЕГО НИК + ID',
                                placeholder='Например: BladeXses#6699 | 045045400656544'))
        self.add_item(InputText(label='БЫЛ ЛИ У ТЕБЯ КЛАН В ПРОШЛОМ?', placeholder='Да/Нет, если да то какой?'))
        self.add_item(InputText(label='НАПИШИ НАЗВАНИЕ любой шрифт И ЦВЕТ КЛАНА #hex',
                                placeholder='Например: ᴅᴇᴠɪʟᴅʏɴᴀꜱᴛʏ | #020202'))
