...
class ClanService:
    ...
    async def drop_menu(self, interaction: discord.Interaction, ctx: ApplicationContext = None):
        ...
        try:
            author_id, my_total_events, my_total_times, my_fault, my_little_fault, my_add_time = cross_event_system.get_clan_staff(ctx.guild.id, ctx.author.id)
            my_butterfly = save_stats_system.get_butterfly(ctx.guild.id, ctx.author.id)

            if ctx.message is None:
                await ctx.response.send_message(
                    embed=StaffProfile(member=ctx.author, total_event=my_total_events, total_time=my_total_times, butterfly=my_butterfly, add_time=my_add_time,
                                       guild=ctx.guild.name, fault=my_fault, little_fault=my_little_fault, icon=ctx.guild.icon, avatar=ctx.author.display_avatar).embed, view=view,
                    ephemeral=True)
            else:
        ...
        async def menu_callback(interact: discord.Interaction):
            ...
            create_profile_view.add_item(drop_down_menu)
            member_id, total_event, total_time, fault, little_fault, add_time = cross_event_system.get_clan_staff(interact.guild.id, drop_down_menu.values[0])
            try:
            ...
            if interact.user.id not in OWNER_IDS:
                await interact.response.edit_message(
                    embed=StaffProfile(member=get_member, total_event=total_event, total_time=total_time, butterfly=get_butterfly, add_time=add_time,
                                       guild=interact.guild.name, fault=fault, little_fault=little_fault, icon=interact.guild.icon, avatar=get_member.display_avatar).embed, view=view)
            else:
                await interact.response.edit_message(
                    embed=StaffProfile(member=get_member, total_event=total_event, total_time=total_time, butterfly=get_butterfly, add_time=add_time,
                                       guild=interact.guild.name, fault=fault, little_fault=little_fault, icon=interact.guild.icon, avatar=get_member.display_avatar).embed,
            ...
            async def edit_fault_callback(inter: discord.Interaction):
                await inter.response.send_message(embed=DefaultEmbed(
                    f'{inter.user.name}, введите количество выговоров, которое хотите прибавить.\nИспользуйте (-) чтобы убавить.'), ephemeral=True)
                ...
                if msg.author.id == inter.user.id:
                    try:
                        number = int(msg.content)
                        cross_event_system.update_fault(guild_id=inter.guild.id, clan_staff_id=member_id, fault=number)
                        await inter.followup.send(embed=DefaultEmbed(f'Успешно изменено на: {msg.content} выговоров. пользователю <@{member_id}>'), ephemeral=True)
                        await msg.delete()
            ...
            edit_profile_view.button_fault.callback = edit_fault_callback
